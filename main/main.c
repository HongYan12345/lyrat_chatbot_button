...
ESP_LOGI(TAG, "[ 1 ] button id=%d", get_input_rec_id());
    ...
    while (1) {
        ...
        if (original_text == NULL) {
            ...
            else if(strstr(original_text, "CO2")){
                typeData = 3;
            }
            else if(strstr(original_text, "luz")){
                typeData = 4;
            }
            else if(strstr(original_text, "luminosidad")){
                typeData = 4;
            }
            ...
            if(strstr(original_text, "hoy")){
                time = 1;
            }
            else if(strstr(original_text, "ayer")){
                time = 2;
            }
            else if(strstr(original_text, "semana")){
                time = 3;
            }
            else if(strstr(original_text, "mes")){
                time = 4;
            }
            else if(strstr(original_text, "año")){
                time = 5;
            }
            ESP_LOGI(TAG, "[ * ] temp_humi: %d, time: %d, range: %d", typeData,time,range);

            text = send_text(typeData, time, range);
            typeData = 0;
            time = 0;
            range = 0;
